...
create table empleado (
		codigo_empleado int(11) primary key not null auto_increment,
...
create table cliente (
		codigo_cliente int(11) primary key not null auto_increment,
...
create table pedido (
		codigo_pedido int(11) primary key not null auto_increment ,
...
select cliente.nombre_cliente , cliente.codigo_empleado_rep_ventas , empleado.nombre , empleado.apellido1 from cliente inner join empleado on cliente.codigo_empleado_rep_ventas = empleado.codigo_empleado;

-- Muestra el nombre de los clientes que hayan realizado pagos junto con el nombre de sus representantes de ventas.
select cliente.nombre_cliente , empleado.nombre from pago inner join cliente on pago.codigo_cliente = cliente.codigo_cliente inner join empleado on cliente.codigo_empleado_rep_ventas = empleado.codigo_empleado;

-- Devuelve el nombre de los clientes que han hecho pagos y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
select cliente.nombre_cliente , empleado.nombre , oficina.ciudad from pago inner join cliente on pago.codigo_cliente = cliente.codigo_cliente inner join empleado on cliente.codigo_empleado_rep_ventas = empleado.codigo_empleado inner join oficina on empleado.codigo_oficina = oficina.codigo_oficina;

-- Devuelve el nombre de los clientes que no hayan hecho pagos y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
select codigo_cliente , nombre_cliente , empleado.nombre , oficina.ciudad from cliente inner join empleado on cliente.codigo_empleado_rep_ventas = empleado.codigo_empleado inner join oficina on empleado.codigo_oficina = oficina.codigo_oficina where codigo_cliente not in (select codigo_cliente from pago);

-- Lista la dirección de las oficinas que tengan clientes en Fuenlabrada.
select cliente.ciudad , oficina.linea_direccion1 from cliente inner join empleado on empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas inner join oficina on oficina.codigo_oficina = empleado.codigo_oficina where cliente.ciudad = 'Fuenlabrada';
